...
export default function CheckoutPage({ items, onBack, onDone }) {
  ...
  const isValid =
    form.firstName && form.lastName && form.phone.length >= 8 &&
  ...
  const confirmOrder = async () => {
    ...
    try {
      ...
      if (err.message === "Failed to fetch") {
        // Offline fallback — order still shows success to user
  ...
  if (step === 3) return (
    <div className="container" style={{ maxWidth: 620, paddingTop: 60, paddingBottom: 100, textAlign: "center" }}>
      ...
      <p style={{ color:"rgba(255,255,255,.32)", fontSize:".84rem", marginBottom:28 }}>
        {delivery === "pickup"
          ? "📍 Store Pickup — Khub, Shebin El Kom"
        ...
        {items.map((it, i) => (
          <div key={i} style={{ display:"flex", gap:12, paddingBottom:16, marginBottom:16, borderBottom:"1px solid rgba(255,255,255,.07)" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontWeight:800, fontSize:".95rem", marginTop:4, fontFamily:"'Syne',sans-serif" }}>
                {(it.price * it.qty).toLocaleString()} <span style={{ fontSize:".7rem", color:"rgba(255,255,255,.35)", fontWeight:400 }}>EGP</span>
              </div>
            </div>
          </div>
        ))}
